...
####################################################################################################################
    ...
    if args.compile_model:
        ...
        gate_stats_data = {
            "layers": [
                {"layer": i,"gates": sort_desc_dict(stats)
                }
        ...
        json_filename = f"./results/{args.experiment_id}_gate_stats_case_2.json"

        with open(json_filename, "w") as f:
            json.dump(gate_stats_data, f, indent=4)

        print(f"Gate statistics saved as JSON: {json_filename}")
