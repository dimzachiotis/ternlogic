...
x_bin = torch.where(x_test < -0.5, -1.0, torch.where(x_test > 0.5, 1.0, 0.0)).float()
...
# with torch.no_grad():
#     out_cuda   = model_cuda(x_bin.cuda()).cpu()
#     out_python = model_python(x_bin)
...
x_bin2 = torch.where(x_test2 < -0.5, -1.0, torch.where(x_test2 > 0.5, 1.0, 0.0)).float()
y_test  = torch.randint(0, num_classes, (8,))

loss_fn = torch.nn.CrossEntropyLoss()

# CUDA backward
out_cuda2 = model_cuda2(x_bin2.cuda())
loss_cuda = loss_fn(out_cuda2, y_test.cuda())
loss_cuda.backward()

# Python backward
out_python2 = model_python2(x_bin2)
