...
class CompiledTernaryBinaryNetMulInputs(torch.nn.Module):
    def __init__(
            self,
        ...
        """
        super(CompiledTernaryBinaryNetMulInputs, self).__init__()
        self.model        = model
        self.device       = device
    ...
    def get_gate_code(self, a_high, a_low, b_high, b_low, gate_id):
        ...
        if gate_id == 0:
            ...
            res_high = f"~(((~{a_high}) | {a_low}))"
            res_low  = f"~(((~{a_low}) | {a_high}))"

        elif gate_id == 18:
            res_high = f"~(((~{b_high}) | {b_low}))"
            res_low  = f"~(((~{b_low}) | {b_high}))"

        elif gate_id == 19:
            res_high = f"~(((~{a_low}) | {a_high}))"
            res_low  = f"~(((~{a_high}) | {a_low}))"

        elif gate_id == 20:
            res_high = f"~(((~{b_low}) | {b_high}))"
            res_low  = f"~(((~{b_high}) | {b_low}))"

        elif gate_id == 21:
            res_high = f"(({b_low} | {b_high}) & ({a_low} | {a_high}))"
            res_low  = z

        elif gate_id == 22:
            res_high = z
            res_low  = f"(({b_low} | {b_high}) & ({a_low} | {a_high}))"

        elif gate_id == 23:
            n0=f"~({b_high} ^ {b_low})"
            n1=f"~({a_high} ^ {a_low})"
            res_high = f"~(({b_low} | {b_high} | {n1}))"
            res_low  = f"~(({a_low} | {a_high} | {n0}))"

        elif gate_id == 24:
            n0=f"~({a_high} ^ {a_low})"
            n1=f"~({b_high} ^ {b_low})"
            res_high = f"~(({a_low} | {a_high} | {n1}))"
            res_low  = f"~(({b_low} | {b_high} | {n0}))"
        
        elif gate_id == 25:
            n0=f"~({b_high} ^ {b_low})"
            n1=f"~({a_high} ^ {a_low})"
            res_high = f"~(({n1} | {n0}))"
            res_low  = f"~(({b_high} | {a_low} | {b_low} | {a_high}))"

        elif gate_id == 26:
            n0=f"~({b_high} ^ {b_low})"
            n1=f"~({a_high} ^ {a_low})"
            res_high = f"~(({b_high} | {a_low} | {b_low} | {a_high}))"
            res_low  = f"~(({n1} | {n0}))"
            ...
            res_high = f"~(({a_low} | {a_high} | {b_low} | {b_high}))"
            res_low  = z

        elif gate_id == 32:
            res_high = f"~((~{a_high}) | (~{b_high}) | {a_low} | {b_low})"
            res_low  = f"~((~{a_low}) | (~{b_low}) | {a_high} | {b_high})"
        
        elif gate_id == 33:
            res_high = f"~((~{a_low}) | (~{b_low}) | {a_high} | {b_high})"
            res_low  = f"~((~{a_high}) | (~{b_high}) | {a_low} | {b_low})"
            ...
            n1=f"~({b_high})"
            res_high = f"~(((~{a_low}) | (~{b_low}) | {a_high} | {b_high}))"
            res_low  = f"~(({b_low} | {n1}) & ({a_low} | {n0}))"

        elif gate_id == 38:
            n0=f"~({a_low})"
            n1=f"~({b_low})"
            res_high = f"~(((~{a_high}) | (~{b_high}) | {a_low} | {b_low}))"
            res_low  = f"~(({b_high} | {n1}) & ({a_high} | {n0}))"

        
        elif gate_id == 39:
            n0=f"~({a_low})"
            n1=f"~({b_low})"
            res_high = f"~(({b_high} | {n1}) & ({a_high} | {n0}))"
            res_low  = f"~(((~{a_high}) | (~{b_high}) | {a_low} | {b_low}))"

        elif gate_id == 40:
            n0=f"~({a_low} & {b_low})"
            n1=f"({a_high} ^ {b_high})"
            res_high  = f"~(({a_low} | {b_low} | {n1}) & ({a_high} | {b_high} | {n0}))"
            res_low = f"~(({res_high}))"
        
        elif gate_id == 41:
            n0=f"~({a_low} & {b_low})"
            n1=f"({a_high} ^ {b_high})"
            res_low  = f"~(({a_low} | {b_low} | {n1}) & ({a_high} | {b_high} | {n0}))"
            res_high = f"~(({res_low}))"
            ...
            n3=f"~(({n1} & {b_low}))"
            n2=f"~(({a_low} ^ {a_high}))"
            res_high  = f"~(({b_low} | {n1} | {n2}) & ({a_high} | {n0} | {n3}))"
            res_low = f"~(((~{a_high}) | {n3} | {a_low}))"
            ...
            n3=f"~(({n1} & {b_low}))"
            n2=f"~(({a_low} ^ {a_high}))"
            res_high  = f"~(((~{a_high}) | {n3} | {a_low}))"
            res_low = f"~(({b_low} | {n1} | {n2}) & ({a_high} | {n0} | {n3}))"
            ...
            n3=f"~(({n1} & {b_high}))"
            n2=f"~(({a_low} ^ {a_high}))"
            res_high  = f"~(({b_high} | {n1} | {n2}) & ({a_low} | {n0} | {n3}))"
            res_low = f"~(((~{a_low}) | {n3} | {a_high}))"
            ...
            n3=f"~(({n1} & {b_high}))"
            n2=f"~(({a_low} ^ {a_high}))"
            res_high  = f"~(((~{a_low}) | {n3} | {a_high}))"
            res_low = f"~(({b_high} | {n1} | {n2}) & ({a_low} | {n0} | {n3}))"
