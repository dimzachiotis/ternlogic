...
def get_all_architectures(exp_name, metric_name, target_lr=None):
    """Extracts runs and averages both accuracy and delay metrics over seeds for each (k, l) pair."""
    ...
    for _, run_row in runs.iterrows():
        ...
        # Extract the delay parameters per run/seed
        delay = safe_float(p.get('delay'))
        delay_2inputs = safe_float(p.get('delay_2inputs'))
        delay_mul = safe_float(p.get('delay_mul_inputs'))
        
        records.append({
            'k': k, 'l': l, 'kl': k * l,
            'delay': delay, 
            'delay_2inputs': delay_2inputs, 
            'delay_mul': delay_mul,
    ...
    # Changed aggregation keys from 'area' to 'delay' to match the records dictionary
    ...
    summary.columns = [
        'k', 'l', 'kl', 
        'mean_acc', 'std_acc', 
        'mean_delay', 'mean_delay_2inputs', 'mean_delay_mul'
    ] 
    return summary
...
# --- 2. Configuration for the two Delay Plots ---
plot_modes = [
    {
...
for mode in plot_modes:
    ...
    # --- Formatting ---
    ax.set_xscale('log')  
    ax.set_xlabel('Mean Hardware Delay (Averaged over Seeds)', fontsize=13)
