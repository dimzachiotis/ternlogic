...
# All 4 possible binary input combinations
bin_vals = [0.0, 1.0]
all_inputs = list(itertools.product(bin_vals, bin_vals))
# shape: [4, 2] — each row is (a, b)
x_test = torch.tensor(all_inputs, dtype=torch.float32)  # [4, 2]
...
def make_single_layer(device, implementation):
    ...
    Indices: neuron i takes input a=0, b=1.
    """
    layer = LogicLayer(
        in_dim=2,
    ...
    layer.indices = (
        torch.zeros(number_of_gates, dtype=torch.int64, device=device),
        torch.ones(number_of_gates,  dtype=torch.int64, device=device),
    )
    ...
    if implementation == 'cuda':
        ...
        for y in range(number_of_gates):
            given_x_indices_of_y[0].append(y)
            given_x_indices_of_y[1].append(y)
...
with torch.no_grad():
    out_cuda   = layer_cuda(x_test.cuda()).cpu()    # [4, 16]
    out_python = layer_python(x_test.clone())       # [4, 16]
...
for gate_i in range(number_of_gates):
    for row_i, (a_val, b_val) in enumerate(all_inputs):
        ...
        expected = bin_op(a, b, gate_i).item()
