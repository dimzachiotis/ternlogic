...
filename = "group1_vs_bin_points.png"
target_lr = 0.01
...
all_tern = get_all_architectures("ternary search lr", "64_tern_testing_acc_3", target_lr=target_lr)
...
# Scatter Ternary
ax.scatter(all_tern['kl'], all_tern['mean_acc'], 
           label=f'Ternary Architectures (LR={target_lr})', 
...
for _, row in all_tern.iterrows():
...
ax.set_ylim(min(all_bin['mean_acc'].min(), all_tern['mean_acc'].min()) - 0.05, 1.0)
